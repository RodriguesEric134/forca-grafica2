...
# Carrega as imagens para os níveis da forca
imagens_forca = [Image.open(f"./img/{i}.png") for i in range(7)]
imagens_forca_tk = [ImageTk.PhotoImage(image) for image in imagens_forca]

# Função para atualizar a imagem da forca
def atualizar_forca():
    imagem_label.config(image=imagens_forca_tk[6 - max_tentativas])
...
imagem_label = tk.Label(root, image=imagens_forca_tk[6])
imagem_label.pack()
