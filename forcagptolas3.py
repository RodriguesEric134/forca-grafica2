...
def atualizar_forca():
    imagem_label.config(image=imagens_forca[6 - max_tentativas])

# Carrega as imagens para os níveis da forca
imagens_forca = [PhotoImage(file=f"./img/{i}.png") for i in range(7)]
...
imagem_label = tk.Label(root, image=imagens_forca[6])
imagem_label.pack()
